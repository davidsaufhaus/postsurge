      ...
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-black/5 bg-white/80 px-6 py-4 backdrop-blur-md">
        <div className="flex items-center gap-8">
          <div>
            ...
            <p className="text-xs text-[#86868b]">Administration &middot; {session?.user?.name}</p>
          </div>
          <nav className="flex gap-5 text-sm font-medium text-[#1d1d1f]/70">
            <Link href="/admin/users" className="transition-colors hover:text-[#0071e3]">
              Nutzerverwaltung
            </Link>
            <Link href="/admin/medications" className="transition-colors hover:text-[#0071e3]">
              Medikamentenkatalog
            </Link>
            <Link href="/admin/templates" className="transition-colors hover:text-[#0071e3]">
              Genesungs-Vorlagen
            </Link>
          </nav>
        </div>
