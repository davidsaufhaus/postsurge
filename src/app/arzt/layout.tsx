import { auth } from "@/auth";
import { LogoutButton } from "@/components/logout-button";
import Link from "next/link";

export default async function ArztLayout({ children }: { children: React.ReactNode }) {
  const session = await auth();

  return (
    <div className="flex flex-1 flex-col">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-black/5 bg-white/80 px-6 py-4 backdrop-blur-md">
        <div className="flex items-center gap-8">
          <div>
            <p className="text-lg font-semibold tracking-tight text-[#1d1d1f]">PostSurge</p>
            <p className="text-xs text-[#86868b]">
              {session?.user?.role === "NURSE" ? "Pflegeansicht" : "Arztansicht"} &middot; {session?.user?.name}
            </p>
          </div>
          <nav className="flex gap-5 text-sm font-medium text-[#1d1d1f]/70">
            <Link href="/arzt" className="transition-colors hover:text-[#0071e3]">
              Dashboard
            </Link>
            <Link href="/arzt/clearance" className="transition-colors hover:text-[#0071e3]">
              Entlassungs-Clearance
            </Link>
            <Link href="/arzt/pflege" className="transition-colors hover:text-[#0071e3]">
              Pflege-Dashboard
            </Link>
          </nav>
        </div>
        <LogoutButton />
      </header>
      <main className="flex-1 bg-[#f5f5f7] p-6">{children}</main>
    </div>
  );
}
